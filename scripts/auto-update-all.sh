...
# 📊 Считаем общее количество папок и шаблонов
total_dirs=($(ls -1 "$srcpkgs_dir" | sort))
total=${#total_dirs[@]}
i=0

echo -e "${GREEN}🔄 Начинаем массовое обновление всех пакетов в $srcpkgs_dir/${NC}"
echo -e "${BLUE}🔍 Найдено пакетов: $total${NC}"
...
for pkgdir in "$srcpkgs_dir"/*/; do
  ...
  old_version=$(grep "^version=" "$template" | cut -d= -f2 | tr -d '"' | sed 's/ //g') || old_version=""
  ...
  # === 2. Извлекаем данные ===
  homepage=$(grep "^homepage=" "$template" | cut -d= -f2- | tr -d '"' | sed 's/ //g') || homepage=""
  distfiles_line=$(grep "^distfiles=" "$template" | sed 's/^distfiles="\(.*\)".*/\1/' || true)
  distfiles=$(echo "$distfiles_line" | awk '{print $1}' | head -1 | sed 's/ //g') || distfiles=""
  ...
  if [[ -n "$homepage" && "$homepage" == https://github.com/* ]] && [[ "$homepage" != *github.com/void-linux* ]]; then
    ...
    new_version=$(curl -s --fail -H "Accept: application/vnd.github.v3+json" "$api_url" | \
      grep '"tag_name":' | head -1 | sed -E 's/.*"([^"]+)".*/\1/' | sed 's/^v//' || true)
  fi
  ...
  # === 7. Обновляем version в template ===
  ...
  archive_url=$(echo "$new_distfiles" | awk '{print $1}' | tr -d '"' | sed 's/ //g') || archive_url=""
